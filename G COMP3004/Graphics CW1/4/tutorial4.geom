...
// In our case this value is 3, since triangles have 3 vertices
 ...
 void main()
 {
    ...
    {
        gl_Position = mvpmatrix * vec4((gl_in[i].gl_Position.xyz - vec3(0., 0., 0.5)), 1.0); ex_Color = geom_Color[i];
        EmitVertex();
    }
    EndPrimitive();
    // Create a new primitive with negative x,y,z values for (i=0; i < gl_in.length(); i++)
    {
        gl_Position = mvpmatrix * vec4((gl_in[i].gl_Position.xyz + vec3(0., 0., 0.5)), 1.0); ex_Color = geom_Color[i];
        EmitVertex();
    }
